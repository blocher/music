...
type SyncRun = { id: number; mode: "incremental" | "full"; status: string; albums_seen: number; tracks_seen: number; loose_tracks_excluded: number; created_at: string; error: string };
...
export function StudioPage() {
  ...
  const [syncingMode, setSyncingMode] = useState<SyncRun["mode"] | "">("");
  ...
  const load = useCallback(async () => {
    ...
    const active = syncRuns[0]?.status === "queued" || syncRuns[0]?.status === "running";
    setSyncing(active);
    setSyncingMode(active ? syncRuns[0].mode : "");
  }, []);
  ...
  const startSync = async (mode: SyncRun["mode"]) => {
    if (mode === "full" && !window.confirm("Refresh every Suno-owned title, description, lyric, source image, duration, and source date? Local audio files and distributor IDs stay in place.")) return;
    setError("");
    setSyncing(true);
    setSyncingMode(mode);
    try {
      const run = await post<SyncRun>("/api/studio/sync-runs/", { mode });
      setRuns((current) => [run, ...current.filter((item) => item.id !== run.id)]);
      await load();
    }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Sync failed."); setSyncing(false); setSyncingMode(""); }
  };
  const latest = runs[0];
    ...
    <div className="studio-page">
      <section className="studio-title-row">
        ...
        <div className="sync-actions">
          <button className="primary-button" disabled={!suno?.connected || syncing} onClick={() => void startSync("incremental")}><RefreshCw className={syncingMode === "incremental" ? "spinning" : ""} />{syncingMode === "incremental" ? "Syncing new songs…" : "Sync new from Suno"}</button>
          <button className="secondary-button" disabled={!suno?.connected || syncing} onClick={() => void startSync("full")}><RefreshCw className={syncingMode === "full" ? "spinning" : ""} />{syncingMode === "full" ? "Refreshing everything…" : "Refresh all from Suno"}</button>
        </div>
      </section>
      {error && <div className="banner error"><CircleAlert />{error}</div>}
      {syncing && <div className="banner sync-banner" role="status" aria-live="polite"><RefreshCw className="spinning" /><span><strong>{latest?.status === "running" ? (syncingMode === "full" ? "Refreshing all Suno metadata…" : "Looking for new Suno songs…") : (syncingMode === "full" ? "Full Suno refresh queued…" : "New-song Suno sync queued…")}</strong><small>You can stay here; albums and song counts update automatically when the sync finishes.</small></span></div>}
      ...
      <section className="studio-metrics">
        <div><span className="eyebrow">Suno sync</span><strong>{latest?.status || "Not run"}</strong><small>{latest ? `${latest.mode === "full" ? "Full refresh" : "New songs only"} · ${new Date(latest.created_at).toLocaleString()}` : "Connect to begin"}</small></div>
        ...
        <div className="studio-album-grid">
          {albums.map((album) => (
            <Link className="studio-album-card" to={`/studio/releases/${album.id}`} key={album.id}>
              ...
              <div><span className="eyebrow">Version {album.version} / {album.status}</span><h3>{album.title}</h3><p>{album.release_date ? new Date(`${album.release_date}T12:00:00`).toLocaleDateString() : "No release date"} · {album.track_count} tracks · {album.public ? "Public" : "Private"}</p></div>
