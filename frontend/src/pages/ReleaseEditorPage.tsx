...
export function ReleaseEditorPage() {
  ...
  if (!album) return <div className="loading-screen">Opening the release desk…</div>;
...
function TracksPanel({ album, reload, confirm }: { album: Album; reload: () => Promise<void>; confirm: React.Dispatch<React.SetStateAction<{ title: string; body: string; action: () => Promise<void> } | null>> }) {
  ...
  return <section className="editor-panel"><header><div><span className="eyebrow">Tracks & source audio</span><h1>{album.title}</h1><p>Order the record and explicitly approve every quota-consuming download.</p></div></header>
...
function LyricsPanel({ album, reload }: { album: Album; reload: () => Promise<void> }) {
  ...
  const sync = async () => { const next = await post<Track>(`/api/studio/tracks/${selected.id}/sync_lyrics/`, {}); setCues(next.timed_lyrics); await reload(); };
  return <section className="editor-panel lyrics-panel"><header><div><span className="eyebrow">Waveform & lyric timing</span><h1>{selected.title}</h1><p>Import LRC, pull Suno alignment, or place and refine each line by hand.</p></div><div className="form-actions"><button className="secondary-button" onClick={() => void sync()}><RefreshCw />Pull Suno timing</button><button className="primary-button" onClick={() => void save()}><Save />Save timing</button></div></header>
    ...
    <div className="timing-wave"><button>▶</button><span>0:00</span><Waveform progress={0.36} /><span>{seconds(Number(selected.duration_seconds || 0))}</span></div>
...
function DeliveryPanel({ album, allTracks, reload, confirm }: { album: Album; allTracks: Track[]; reload: () => Promise<void>; confirm: React.Dispatch<React.SetStateAction<{ title: string; body: string; action: () => Promise<void> } | null>> }) {
  ...
  const submit = () => confirm({ title: `Publish ${album.title} through Too Lost?`, body: "This creates a real distributor submission from the immutable metadata snapshot shown here. Store delivery cannot be undone from this screen.", action: async () => { await post<Submission>("/api/studio/submissions/", { album: album.id, kind: album.replaces_id ? "replacement" : "new" }); await reload(); } });
  ...
  return <section className="editor-panel delivery-panel"><header><div><span className="eyebrow">Distribution</span><h1>Send the record into the world.</h1><p>Validate first. Submission and replacement are separate, confirmed operations.</p></div></header>
...
function LinksPanel({ album, reload }: { album: Album; reload: () => Promise<void> }) {
  ...
  return <section className="editor-panel"><header><div><span className="eyebrow">Store destinations</span><h1>One record, every listening room.</h1><p>Add the artist, album, and song URLs returned after delivery.</p></div></header>
